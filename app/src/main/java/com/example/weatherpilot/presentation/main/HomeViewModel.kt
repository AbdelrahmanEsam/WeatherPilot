...
class HomeViewModel @Inject constructor(
    ...
    private val getWeatherFromRemoteResponseUseCase: GetWeatherFromRemoteResponseUseCase
) : ViewModel() {


    private val _stateDisplay: MutableStateFlow<HomeState.Display> =
        MutableStateFlow(HomeState.Display())
    val stateDisplay = _stateDisplay.asStateFlow()


    private val _stateLongLat: MutableStateFlow<HomeState.LongLat> =
        MutableStateFlow(HomeState.LongLat())
    val stateLongLat = _stateLongLat.asStateFlow()


    private val _statePreferences: MutableStateFlow<HomeState.Preferences> =
        MutableStateFlow(HomeState.Preferences())
    ...
    fun onEvent(intent: HomeIntent) {
        when (intent) {
            ...
            is HomeIntent.FetchDataOfFavouriteLocation -> {
                ...
                getFavouriteWeatherResponse()
            }

            HomeIntent.ReadLatLongFromDataStore -> readLocationLatLonFromDataStore()
            HomeIntent.FetchData -> {
                if (statePreferences.value.locationType.equals("favourite")) {
                    getFavouriteWeatherResponse()
                } else {
                    getWeatherResponse()
                }
            ...
            stateLongLat.value.longitude?.let {
                ...
                val weatherResponse =
                    getWeatherDataUseCase.execute(
                        longitude = stateLongLat.value.longitude!!,
            ...
            when (response) {
                is Response.Success -> {
                    if (local == "ar") {
                        ...
                        val localCityName: String =
                            (response.data as SearchResponse).searchResults.first()?.LocalNames?.ar
                                ?: response.data.searchResults.first()?.LocalNames?.en ?: "en"
                        _stateDisplay.update { it.copy(city = localCityName) }
                       updateCityNameUseCase.execute<Response<String>>(
                            _stateDisplay.value.city!!
                        )
                    } else {
                    ...
                    _stateDisplay.update { it.copy(city = city) }
                }
            }
        }
    }
    ...
    private fun getFavouriteWeatherResponse() {
        viewModelScope.launch(ioDispatcher) {
            getWeatherFromRemoteResponseUseCase.execute(
                longitude = stateLongLat.value.longitude!!,
                latitude = stateLongLat.value.latitude!!,
                statePreferences.value.languageType ?: "en"
